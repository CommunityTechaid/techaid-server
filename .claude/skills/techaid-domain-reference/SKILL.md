...
- `ReferringOrganisation`: name, website, phoneNumber, `archived` (Y/N). `requestCount` `@Formula` counts **open** requests across its contacts — those **NOT IN** (`REQUEST_CANCELLED`,`REQUEST_COMPLETED`,`REQUEST_DECLINED`). It counted only `status='NEW'` until PR #113; since `NEW` is transient (nothing rests in it), that rendered 0 for every organisation.
...
- **Open question — read issue #120 before changing either formula.** Both count `REQUEST_COLLECTION_DELIVERY_FAILED` as open; the project's two other definitions of "open" (`DeviceRequestRepository.requestCount()`'s native query and `DeliveryAdminQueries.CLOSED_REQUEST_STATUSES`) both exclude it. Reconciling them was tried in PR #118 and reverted in PR #121: because the contact-level formula gates who may raise a request, which side that status falls on is a policy decision rather than a consistency fix. `RequestCountSemanticsTest` pins the current classification of every status.
