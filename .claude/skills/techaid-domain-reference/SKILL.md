...
- **"Open request" has exactly one definition: `CLOSED_REQUEST_STATUSES` in `DeviceRequestModels.kt`** — a request is closed iff its status is `REQUEST_COMPLETED`, `REQUEST_DECLINED` or `REQUEST_CANCELLED`; everything else is open. Its twin `CLOSED_REQUEST_STATUSES_SQL` is the same set as a SQL literal, because Hibernate `@Formula` and Spring Data `@Query` are annotations whose arguments must be compile-time constants and enum property access is not one (a `const val` string is). Don't reintroduce a hardcoded status list — use one of those two. `RequestCountSemanticsTest` asserts the SQL literal and the Kotlin set never drift.
- **`REQUEST_COLLECTION_DELIVERY_FAILED` is OPEN.** Settled by the team on 2026-07-30 (Cat Smith, Steve Woolnough, Mahi Nair — issue #120): the referrer gets two weeks to rebook, the device stays assigned until they do, and only then is the request closed by hand — so it should keep consuming one of their three slots and keep the onus on them. PR #118 assumed the opposite (closed) and was reverted in #121; the settled answer went the other way, bringing the demand query and the delivery-slots screen up to match the formulas rather than the reverse.
- `ReferringOrganisation`: name, website, phoneNumber, `archived` (Y/N). `requestCount` `@Formula` counts **open** requests across its contacts. It counted only `status='NEW'` until PR #113; since `NEW` is transient (nothing rests in it), that rendered 0 for every organisation.
- `ReferringOrganisationContact`: fullName, email, phoneNumber, address, `archived`. Its `requestCount` `@Formula` applies the same closed set to that one contact, and **this is the count the 3-request limit checks** (`DEVICE_REQUEST_LIMIT`). The org-level and contact-level formulas must be kept in step — the same field name has to mean the same thing at both levels (PR #113).
- The three places that draw the open/closed line, all now sharing the constant: `DeviceRequestRepository.requestCount()` (system-wide outstanding device demand, the dashboard landing page), the two `requestCount` `@Formula` fields (org/contact records **and** the 3-request cap), and `DeliveryAdminQueries` (`matchedRequestOpen`, the "closed" badge on the delivery-slots screen).
